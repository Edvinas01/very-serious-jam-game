...
namespace InSun.GameCore.Editor.SunnyInspector.Utilities
{
    /// <summary>
    /// See https://github.com/dbrizov/NaughtyAttributes/blob/master/Assets/NaughtyAttributes/Scripts/Editor/Utility/PropertyUtility.cs#L12
    /// </summary>
    internal static class PropertyUtilities
    {
        ...
        {
            FieldInfo fieldInfo = ReflectionUtilities.GetField(GetTargetObjectWithProperty(property), property.name);
            if (fieldInfo == null)
            {
                return new T[] { };
            }
        ...
        internal static List<bool> GetConditionValues(object target, string[] conditions)
        {
            ...
            {
                FieldInfo conditionField = ReflectionUtilities.GetField(target, condition);
                if (conditionField != null && conditionField.FieldType == typeof(bool))
                {
                    conditionValues.Add((bool)conditionField.GetValue(target));
                }

                PropertyInfo conditionProperty = ReflectionUtilities.GetProperty(target, condition);
                ...
                MethodInfo conditionMethod = ReflectionUtilities.GetMethod(target, condition);

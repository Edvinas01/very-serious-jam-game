...
namespace CHARK.BuildTools.Editor
{
    ...
    internal static class PlayerBuilder
    {
        ...
        private static readonly IReadOnlyList<string> BuildDebugInformationDirectorySuffixes = new List<string>
        {
        ...
        private static void Archive(BuildOptions buildOptions)
        {
            ...
            if (buildOptions.BuildTarget == BuildToolsTarget.WebGL)
            {
                Archive(
                    Path.Combine(Path.GetDirectoryName(buildPath), Path.GetFileNameWithoutExtension(buildPath)),
                    archiveFilePath
                );
            }
            else
        ...
        private static string GetArchiveFilePath(BuildOptions buildOptions)
        {
            ...
            var archiveFileName = $"{string.Join(settings.ArchiveDelimiter, parts)}." + $"{BuildArchiveExtension}";

            if (settings.IsLowerCaseArchiveName)
            {
                archiveFileName = archiveFileName.ToLower();
            }

            if (settings.IsDashArchiveName)
            {
        ...
        private sealed class ArchiveDirectoryFilter : IScanFilter
        {
            public bool IsMatch(string name)
            {
                foreach (var suffix in BuildDebugInformationDirectorySuffixes)
                {
